...
namespace tree {
    ...
    class CNode {
        public:
            int visit_count, to_play, latent_state_index_in_search_path, latent_state_index_in_batch, best_action;
            ...
            void expand(int to_play, int latent_state_index_in_search_path, int latent_state_index_in_batch, float reward, const std::vector<float> &policy_logits);
    ...
    void cbatch_backpropagate(int latent_state_index_in_search_path, float discount_factor, const std::vector<float> &rewards, const std::vector<float> &values, const std::vector<std::vector<float> > &policies, tools::CMinMaxStatsList *min_max_stats_lst, CSearchResults &results, std::vector<int> &to_play_batch);
