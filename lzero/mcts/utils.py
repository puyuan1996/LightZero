...
def obtain_tree_topology(root, to_play=-1):
    ...
    while len(node_stack) > 0:
        ...
        node_stack.pop()
        node_dict = {}
        node_dict['node_id'] = node.latent_state_index_in_search_path
        ...
        node_id_list.append(node.latent_state_index_in_search_path)
        for a in node.legal_actions:
            child = node.get_child(a)
            if child.expanded:
                child.parent_latent_state_index_in_search_path = node.latent_state_index_in_search_path
                edge_dict = {}
                edge_dict['parent_id'] = node.latent_state_index_in_search_path
                edge_dict['child_id'] = child.latent_state_index_in_search_path
