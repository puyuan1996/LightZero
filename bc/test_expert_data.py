        ...
        if get_rank() == 0:
            ...
            with self._timer:
                while not eval_monitor.is_finished():
                    ...
                    try:
                        print("Obs dist: " +
                              str(torch.sum(obs[0]['observation'] != (self.real_dataset[cnt]['obs'])).item()))
                        print((obs[0]['observation'] != (self.real_dataset[cnt]['obs'])).shape)
                    except:
                        print("no action")
                        pass
                    # update videos
...
def test_main(
    ...
    # Random seed
    evaluator_env.seed(0, dynamic_seed=False)
